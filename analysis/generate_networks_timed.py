...
parser = argparse.ArgumentParser(
    description='This script can generate two network types: directed network or the '
    'undirected network per date. In either case, only the posts within the past x days '
    'will be considered, where x is specified by the user hd flag. We will take snapshot '
    'of the network whenever we observe a new date in the posts. Edge construction is '
    'different in directed and undirected networks. Read file level comments for '
    'details.')
...
parser.add_argument("-hd", "--history_days", type=int, default = 30,
                    ...
                    "network.")
parser.add_argument("-nc" , "--num_coopearing_users", dest="num_coopearing_users",
                    type=int, default=20,
                    help="If we are creating undirected neworks, this flag will "
                    "determine how many of the last posting users in the thread will "
                    "receive an edge to the most recent poster. The default value is 20 "
                    "which means any poster will become connected only to the previous "
                    "20 posters in the same thread. The choice is made since the UI "
                    "shows 20 posts per thread in one page.")
...
class InteractionGraph:
  ...
  """
  def __init__(self):
    # a dictionary from "forum_id,subject_id" to a FIFO queue of users who contributed in
    # it. Use this to update the undirected interactions graph. Only contains the *last*
    # num_coopearing users who have posted in the thread. It will be used in case of
    # undirected network to connect the most recent poster in a thread to the previous
    # posters in the thread.  The number of the immediate last posters will be defined by
    # num_coopearing_users flag.
    self.subject_contributors = collections.defaultdict(
        lambda: collections.deque(maxlen=args.num_coopearing_users))
  ...
  def add_post(self, full_subject_id, user, started_by):
    ...
    # user who originated the thread (directed) or latest contributors (undirected).
    # Weights are updated according to thread activity.
    if args.directed_network:
      utils.update_edge_weights_directed(user,
                                         started_by,
                                         args.weigh_thread,
                                         self.subject_activity[full_subject_id],
                                         self.edge_weights)
    else:
      utils.update_edge_weights_undirected(user,
                                           args.weigh_thread,
                                           self.subject_contributors[full_subject_id],
                                           self.edge_weights)
    
    # add user to the post contributors
    (self.subject_contributors[full_subject_id]).append(user)
