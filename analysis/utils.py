...
def extract_fields_from_row(input_row):
  ...
  date = input_row[12]
  time = input_row[13]
  num_quotes = input_row[14]
  #num_tokens = input_row[15]
  #num_sentimental_tokens = input_row[16]
  dictionary_absolute_sentiment_score = input_row[15]
  dictionary_relative_sentiment_score_all_tokens = input_row[16]
  dictionary_relative_sentiment_score_sentimental_tokens = input_row[17]
  mentioned_modified_coins = [x.upper() for x in input_row[18].split('|') if x]
  mentioned_unmodified_coins = [x.upper() for x in input_row[19].split('|') if x]
...
def read_forum_data(forum_input_file):
  posts = pd.read_csv(forum_input_file,
                      usecols=['forum_id','subject_id','post_id','num_replies',
                               'num_views','url','started_by','user','user_level',
                               'user_activity','date', 'time',
                               'num_quotes', 'num_tokens', 'num_sentimental_tokens',
                               'dictionary_absolute_sentiment_score',
                               'dictionary_relative_sentiment_score_all_tokens',
                               'dictionary_relative_sentiment_score_sentimental_tokens'])
  posts['thread_id'] = posts['forum_id'].map(str) + '-' + posts['subject_id'].map(str)
  posts['full_id'] = posts['thread_id'].map(str) + '-' + posts['post_id'].map(str)
  posts['date_time'] = pd.to_datetime(posts['date'] + ' ' + posts['time'])
  posts['date'] = pd.to_datetime(posts['date'])
  posts = posts.set_index('full_id')
  posts = posts.sort_values(by='date_time')
  assert posts.index.is_unique
  return posts


# creates a new edge or updates the weight of the edge in edge_weights.
# edge_weights is a dict of edges keyed by (source, target).
# subject_activity is the level activity in the thread where this edge/post comes from.
# subject_contributors is the deque last contributors in the thread where this edge/post
# comes from.
#
...
# influence and power.  Weights are updated according to thread activity.
def update_edge_weights_directed(user,
                                 started_by,
                                 weigh_thread,
                                 subject_activity,
                                 edge_weights):
...
def update_edge_weights_undirected(user,
                                   weigh_thread,
                                   subject_contributors,
                                   edge_weights):
  # we don't want to add more than one edge to the same contributor, in case the same
  # contributor appeared more than once in the previous posts. So we keep track of users
  # to whom we already added an edge. also don't add an edge from user to herself.
  unique_contributors = set([user])

  # To Start with the most recent contributor, the deque should be reversed, since the
  # first element is the oldest poster
  for num_contributor, contributor in enumerate(reversed(subject_contributors), start=1):
    # add an edge only we have not yet added an edge to this contributor from previous
    # posts
    if contributor in unique_contributors:
      continue
