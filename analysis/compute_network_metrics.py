...
parser.add_argument("-n", "--num_processes", dest="num_processes",
                    ...
                    "network metrics.")
parser.add_argument("-s", "--sampling_rate", dest="sampling_rate",
                    type = float, default = 0.1,
                    help="The percentage of vertices to sample for computing the moments "
                    "of each metric distribution among all the other nodes in the "
                    "network. Default is 10 percent.")
parser.add_argument("-m", "--max_users_per_coin", dest="max_users_per_coin",
                    type = int, default = 1,
                    help="Maximum number of users for which to compute network measures "
                    "per coin. The actual number of users per coin in the output file "
                    "could be less than this, in case the users_input_file has less than "
                    "this many users.")
...
def read_coin_users(users_input_file):
  '''
  returns:
  max_num_coin_users: the size of largest active user set among all coins. For some coins,
  the input file might hav only one active user, for others the input file might have more
  than 100. This will be largest num_users per coin.
  coin_users: mapping from coin to list of coin active/introducing users and their stats.
  '''
...
def compute_mean_median_std_laplacian_centrality(network):
  # find mapping from vertex to sum of edge weights
  vertex_weights = collections.defaultdict(float)
  for edge in network.es:
    source = edge["source"]
    ...
    vertex_weights[target] += weight
  s = []
  for vertex in network.vs["name"]:
    neighbors = network.neighbors(vertex, mode="all")
    s.append(vertex_weights[vertex]**2 + vertex_weights[vertex] +
             2 * sum(vertex_weights[i] for i in neighbors) )
...
# network.
def compute_user_satoshi_distance(network, directed_networks, user, weighted):
...
def compute_mean_median_std_satoshi_distance(network, directed_networks, weighted):
  if 'satoshi' not in network.vs["name"]:
    return [float("inf")]*3

  sample_vertices = numpy.random.choice(network.vs["name"],
                                        size=args.sampling_rate*len(network.vs),
  ...
  s = network.shortest_paths(source='satoshi', target=sample_vertices, weights=weights, mode=mode)[0]
  if any([numpy.isinf(x) for x in s]):
...
def compute_user_satoshi_pagerank(network, user, directed_networks, weighted):
  if len(network.es) == 0:
    return 1
  if 'satoshi' not in network.vs["name"]:
    return 0
  ...
  user_satoshi_pagerank = network.personalized_pagerank(
      directed=directed_networks, damping=0.85, reset_vertices='satoshi', weights=weights,
...
def compute_mean_median_std_satoshi_pagerank(network, directed_networks, weighted):
  if len(network.es) == 0:
    return [1]*3
  if 'satoshi' not in network.vs["name"]:
    return [0]*3
  ...
  s = network.personalized_pagerank(
      directed=directed_networks, damping=0.85, reset_vertices='satoshi', weights=weights,
      implementation = "prpack", arpack_options = options)
  return [numpy.mean(s), numpy.median(s), numpy.std(s)]
...
def compute_user_degree(network, user, mode):
  return network.degree(user, mode, loops=False)


def compute_mean_median_std_degree(network, mode):
  sample_vertices = numpy.random.choice(network.vs["name"],
                                        size=args.sampling_rate*len(network.vs),
                                        replace=False).tolist()
  s = network.degree(sample_vertices, mode, loops=False)
  return [numpy.mean(s), numpy.median(s), numpy.std(s)]
...
def compute_mean_median_std_betweenness_centrality(network, directed_networks, weighted):
  if len(network.es) == 0:
    return [1]*3
  ...
  s = network.betweenness(vertices = sample_vertices,
                          directed = directed_networks,
                          weights = weights,
                          nobigint = False)
  return [numpy.mean(s), numpy.median(s), numpy.std(s)]
...
def compute_mean_median_std_closeness_centrality(network, mode, weighted):
  sample_vertices = numpy.random.choice(network.vs["name"],
                                        size=args.sampling_rate*len(network.vs),
                                        replace=False).tolist()
  if weighted:
    ...
    weights = None
  s = network.closeness(vertices = sample_vertices,
                        mode = mode,
                        weights = weights)
  return [numpy.mean(s), numpy.median(s), numpy.std(s)]
...
def compute_network_metrics(network_info):
  ...
  if not args.skip_general_metrics:
    ...
    # median/mean/std metrics
    result.extend(compute_mean_median_std_degree(network, igraph.ALL))
    result.extend(compute_mean_median_std_closeness_centrality(network, igraph.ALL, False))
    result.extend(compute_mean_median_std_closeness_centrality(network, igraph.ALL, True))
    #result.extend(compute_mean_median_std_betweenness_centrality(network, directed_networks, True))
    result.extend(compute_mean_median_std_satoshi_distance(network, directed_networks, False))
    result.extend(compute_mean_median_std_satoshi_distance(network, directed_networks, True))
    result.extend(compute_mean_median_std_satoshi_pagerank(network, directed_networks, False))
    result.extend(compute_mean_median_std_satoshi_pagerank(network, directed_networks, True))
    result.extend(compute_mean_median_std_pagerank(network, directed_networks, False))
    result.extend(compute_mean_median_std_pagerank(network, directed_networks, True))
    if args.directed_networks:
      result.extend(compute_mean_median_std_degree(network, igraph.IN))
      result.extend(compute_mean_median_std_degree(network, igraph.OUT))
      result.extend(compute_mean_median_std_closeness_centrality(network, igraph.IN, False))
      result.extend(compute_mean_median_std_closeness_centrality(network, igraph.OUT, False))
      result.extend(compute_mean_median_std_closeness_centrality(network, igraph.IN, True))
      result.extend(compute_mean_median_std_closeness_centrality(network, igraph.OUT, True))
    else:
      result.extend(compute_mean_median_std_laplacian_centrality(network))
  ...
  if not args.skip_user_metrics:
    ...
    num_user_fields = 21 if directed_networks else 16
    users = coin_users[network_name]
    num_users_per_coin = min(max_num_coin_users, args.max_users_per_coin)
    for i in range(0, num_users_per_coin):
      ...
      user = users[i][0]
      if user not in network.vs["name"]:
        print("Error: user %s for coin %s with earliest mention date %s not in the "
              "network %s. Skipping this coin." %
              (user, network_name, earliest_mention_date, network_date))
        return
      ...
      user_info.append(compute_user_clustering_coefficient(network, user))
      user_info.append(compute_user_degree(network, user, igraph.ALL))
      user_info.append(compute_user_closeness_centrality(network, user, igraph.ALL, False))
      user_info.append(compute_user_closeness_centrality(network, user, igraph.ALL, True))
      # betweenness take a very long time, so it can be commented out
      #user_info.append(compute_user_betweenness_centrality(network, user, directed_networks, True))
      ...
      user_info.append(compute_user_satoshi_distance(network, directed_networks, user, False))
      user_info.append(compute_user_satoshi_distance(network, directed_networks, user, True))
      user_info.append(compute_user_satoshi_pagerank(network, user, directed_networks, False))
      user_info.append(compute_user_satoshi_pagerank(network, user, directed_networks, True))
      user_info.append(compute_user_pagerank(network, user, directed_networks, False))
      user_info.append(compute_user_pagerank(network, user, directed_networks, True))
      
      if directed_networks:
        user_info.append(compute_user_degree(network, user, igraph.IN))
        user_info.append(compute_user_degree(network, user, igraph.OUT))
        user_info.append(compute_user_closeness_centrality(network, user, igraph.IN, False))
        user_info.append(compute_user_closeness_centrality(network, user, igraph.OUT, False))
        user_info.append(compute_user_closeness_centrality(network, user, igraph.IN, True))
        user_info.append(compute_user_closeness_centrality(network, user, igraph.OUT, True))
      else:
        user_info.append(compute_laplacian_centrality(network, user))
...
def main():
  ...
  if not args.skip_general_metrics:
    header.extend(["num_vertices", "num_edges",
                   ...
                   #"weighted_average_path_length",
                   "clustering_coefficient",
                   "mean_degree", "median_degree", "std_degree",
                   "mean_closeness_centrality_unweighted", "median_closeness_centrality_unweighted", "std_closeness_centrality_unweighted",
                   "mean_closeness_centrality_weighted", "median_closeness_centrality_weighted", "std_closeness_centrality_weighted",
                   #"mean_betweenness_centrality_weighted", "median_betweenness_centrality_weighted", "std_betweenness_centrality_weighted",
                   "mean_satoshi_distance_unweighted", "median_satoshi_distance_unweighted", "std_satoshi_distance_unweighted",
                   "mean_satoshi_distance_weighted", "median_satoshi_distance_weighted", "std_satoshi_distance_weighted",
                   "mean_satoshi_pagerank_unweighted", "median_satoshi_pagerank_unweighted", "std_satoshi_pagerank_unweighted",
                   "mean_satoshi_pagerank_weighted", "median_satoshi_pagerank_weighted", "std_satoshi_pagerank_weighted",
                   "mean_pagerank_unweighted", "median_pagerank_unweighted", "std_pagerank_unweighted",
                   "mean_pagerank_weighted", "median_pagerank_weighted", "std_pagerank_weighted"])
    if args.directed_networks:
      header.extend(["mean_degree_incoming", "median_degree_incoming", "std_degree_incoming",
                     "mean_degree_outgoing", "median_degree_outgoing", "std_degree_outgoing",
                     "mean_closeness_centrality_incoming_unweighted", "median_closeness_centrality_incoming_unweighted", "std_closeness_centrality_incoming_unweighted",
                     "mean_closeness_centrality_outgoing_unweighted", "median_closeness_centrality_outgoing_unweighted", "std_closeness_centrality_outgoing_unweighted",
                     "mean_closeness_centrality_incoming_weighted", "median_closeness_centrality_incoming_weighted", "std_closeness_centrality_incoming_weighted",
                     "mean_closeness_centrality_outgoing_weighted", "median_closeness_centrality_outgoing_weighted", "std_closeness_centrality_outgoing_weighted"])
    else:
      header.extend(["mean_laplacian_centrality_weighted", "median_laplacian_centrality_weighted", "std_laplacian_centrality_weighted"])
  ...
  if args.users_input_file:
    ...
    if not args.skip_user_metrics:
      ...
      num_users_per_coin = min(max_num_coin_users, args.max_users_per_coin)
      for i in range(1, num_users_per_coin + 1):
        user = "user" + str(i)
        header.extend([user,
                       user + "_num_mentions",
                       user + "_num_posts",
                       ...
                       user + "_satoshi_distance_unweighted",
                       user + "_satoshi_distance_weighted",
        ...
        if args.directed_networks:
          header.extend([user + "_degree_incoming",
          ...
          header.extend([user + "_laplacian_centrality_weighted"])
  else:
